...
for testParam in testList:
	index = 0
		...
		saveSuffix = out_pre+spec
                ...
                options = optionsCommon + ' --testParam ' + testParam + ' --testVal ' + str(testVal) + ' --output ' + saveRoot+saveSuffix + ' --index ' + str(index)
                nowCommand += ' python '+os.environ['FISHER_DIR']+'/bin/cluster-submit/testStepSize_driver_script.py '+options+';'
